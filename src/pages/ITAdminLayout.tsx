...
const ITAdminLayout = () => {
  ...
  useEffect(() => {
    const loadUser = async () => {
      try {
        ...
        setUser(currentUser);
        console.log('🏠 ITAdminLayout loaded user:', currentUser);
      } catch (error) {
        console.error('ITAdminLayout error loading user:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const isActiveRoute = (path: string) => {
    if (path === '/itadmin') {
      return location.pathname === '/itadmin';
    }
  ...
  return (
    <div className="min-h-screen relative"
      ...
      <div className="relative z-10 flex min-h-screen">
        {/* Sidebar Navigation - Traditional Design */}
        <div className="w-64 bg-white flex flex-col">
          ...
          <div className="p-4">
            <div className="space-y-1">
              ...
              <button 
                onClick={() => navigate('/itadmin/requests')}
                className={`w-full px-4 py-3 text-left rounded-lg transition-colors flex items-center space-x-3 ${
                  isActiveRoute('/itadmin/requests')
                    ? 'bg-blue-50 text-blue-700 border-l-4 border-blue-500' 
                    : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
              ...
              <button 
                onClick={() => navigate('/itadmin/campaigns')}
                className={`w-full px-4 py-3 text-left rounded-lg transition-colors flex items-center space-x-3 ${
                  isActiveRoute('/itadmin/campaigns')
                    ? 'bg-blue-50 text-blue-700 border-l-4 border-blue-500' 
                    : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
              ...
              <button 
                onClick={() => navigate('/itadmin/applications')}
                className={`w-full px-4 py-3 text-left rounded-lg transition-colors flex items-center space-x-3 ${
                  isActiveRoute('/itadmin/applications')
                    ? 'bg-blue-50 text-blue-700 border-l-4 border-blue-500' 
                    : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
