-- Refresh schema cache and verify functions
...
FROM pg_proc 
WHERE proname IN ('approve_request_sequential', 'reject_request_sequential')
AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public');

-- Drop and recreate functions to ensure they're properly deployed
DROP FUNCTION IF EXISTS approve_request_sequential(UUID, VARCHAR(255), TEXT);
DROP FUNCTION IF EXISTS reject_request_sequential(UUID, VARCHAR(255), TEXT);

-- Recreate approve function
